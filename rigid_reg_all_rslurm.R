...
k=k+1
  for (i in sub_list) {  
    ...
    m3=paste("mi[",path_to_dz,"lhtemplate2.nii.gz,",path_to_cpb,i,"-t2s-mask.nii.gz, 1, 32,Regular, 0.25]",sep="")
    
    #set output path and file pre-fix

    output=paste(path_to_dz,i,"-30-pass",k,sep="")
    df= data.frame(1:1)
    pars = list(list(
      d = 3, #3 dimentions
...
#}
sjob = slurm_apply(antsRegistration, df_full,jobname="AntsRegT", nodes = 30, cpus_per_node = 4)